# coding: utf-8
#!/usr/bin/env python
# TODO: setting.py  np.set_printoptions(threshold=25)
"""
...
def make_timeseries_regressor(window_size, filter_length, nb_input_series=1, nb_outputs=1, nb_filter=4):
    ...
    :param int window_size: The number of previous timeseries values to use as input features.  
    ...
    model = Sequential((
        ...
        # Convolution1D(nb_filter=フィルタの数, filter_length=スライド幅, activation='relu', input_shape=(N回前まで, 入力次元数)),
        MaxPooling1D(),
...
def main():
    ...
    ts_length = 1000
    window_size = 50
    timeseries1 = [int(np.sin(i) > 0) for i in np.arange(ts_length) / float(100)]
    timeseries2 = [int(np.cos(i) > 0) for i in np.arange(ts_length) / float(100)]

    print('\nSimple single timeseries vector prediction')
    # evaluate_timeseries(timeseries1, window_size)

    print('\nMultiple-input, multiple-output prediction')
    timeseries = np.array([timeseries1, timeseries2]).T      # The timeseries f(t) = [t, -t]
